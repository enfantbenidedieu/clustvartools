# -*- coding: utf-8 -*-
from numpy import ones, mean, array, linalg, ndarray, average, cov, sqrt, unique, where
...
def eta2(categories, values):
    categories = array(categories)
    values = array(values)
    ssw, ssb = 0, 0
    for category in set(categories):
        subgroup = values[where(categories == category)[0]]
        ssw += sum((subgroup-mean(subgroup))**2)
        ssb += len(subgroup)*(mean(subgroup)-mean(values))**2
    return (ssb / (ssb + ssw))
...
def wvar(X, w=None,ddof=0):  
    ...
    X : array-like of shape (n_samples,) or (n_samples, n_columns)
        Input data containing the data to be averaged. 
        X must be an object of class ``pandas.Series`` or ``pandas.DataFrame``.
...
def wstd(X, w=None, ddof=0):  
    ...
    return Series(sqrt([cov(m=X.iloc[:,j],aweights=w,ddof=ddof) for j in range(X.shape[1])]),index=X.columns,name="scale").astype("float")
...
def wcorr(X, w=None, ddof=0) -> DataFrame:
    """
...
def wpcorr(X,partial=None,w=None, ddof=0) -> DataFrame:
    """
...
def func_groupby(X, by, func="mean", w=None, ddof=0) -> DataFrame:
    """
