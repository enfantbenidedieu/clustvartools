...
#intern functions
from .utils import is_object_or_category_dtype
...
def preprocessing(X):
    ...
    if not X_quanti.empty:
        X[X_quanti.columns] = SimpleImputer(strategy="mean").fit_transform(X_quanti)
    ...
    if not X_quali.empty:
        Y = SimpleImputer(strategy="most_frequent").fit_transform(X_quali)
        Y = DataFrame(Y,index=X.index,columns=X_quali.columns)
        X[X_quali.columns] = revalue(X=Y)
