...
def eta2test(x,y,w=None) -> NamedTuple:
    """
    Test of Eta squared
...
def association(X):
    ...
    for i in range(X.shape[1]-1):
        for j in range(i+1,X.shape[1]):
            ...
            init_dict = {"variable1":X.columns[i],"variable2":X.columns[j]}
            #contingency table
            tab = crosstab(X.iloc[:,i],X.iloc[:,j])
            #pearson chi-squared test
            statistic, pvalue, dof, _ = chi2_contingency(tab,lambda_=None,correction=False)
            row_chi2 = DataFrame({**init_dict,**{"statistic":statistic,"dof":dof,"pvalue":pvalue}},index=[idx])
            ...
            row_gtest = DataFrame({**init_dict,**{"statistic":g_stat,"dof":g_dof,"pvalue":g_pvalue}},index=[idx])
            ...
            row_asso = DataFrame({**init_dict,**{"cramer":asso_test[0],"tschuprow":asso_test[1],"pearson":asso_test[2]}},index=[idx])
...
def wcorrtest(X,w=None) -> DataFrame:
    """
    ...
    for i in range(n_cols-1):
        for j in range(i+1,n_cols):
            ...
            row_corr = DataFrame({**init_dict,**{"test":test,"statistic":res.statistic,"pvalue":res.pvalue}},index=[idx])
