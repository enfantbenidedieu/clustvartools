...
class CLVmix(BaseEstimator,TransformerMixin):
    """
    Hierarchical Clustering of Mixed Variables around Latent Variables (CLVmix)
    
    Ascendant hierarchical clustering of a set of variables. Variables are mixture of both. 
    The aggregation criterion is the decrease in homogeneity for the clusters being merged. 
    The homogeneity of a cluster is the sum of the correlation ratio (for categorical variables) 
    and the squared correlation (for continuous variables) between the variables and the center of the cluster 
    which is the first principal component of factor analysis of mixed data (FAMD/PCAmix). 
    FAMD/PCAmix are defined for a mixture of categorical and continuous variables and 
    includes ordinary principal component analysis (PCA) and multiple correspondence analysis (MCA) as special cases. 
    ...
    var_ : var
        ...
        cor : DataFrame of shape (n_quanti, ncl)
            Peearson correlation of variables and latent components.
        cortest : DataFrame of shape (n_quanti*ncl, 6)
    ...
    def fit(self,X,y=None):
        ...
        #---------------------------------------------------------------------------------------------------------------------------------------------------------------------
        #initialize
        Xcod, center, scale, dummies = None, None, None, None
        # set continuous variables
        if n_quanti > 0:
            #compute weighted average and standard deviation
            center1 = wmean(X_quanti,w=row_w)
            scale1 = wstd(X_quanti,w=row_w)
            #concatenate
            Xcod = concat_empty(Xcod,X_quanti,axis=1)
            center = concat_empty(center,center1,axis=0)
            scale = concat_empty(scale,scale1,axis=0)
            
        #set categoricals variables weights
        if n_quali > 0:
            # disjunctive table
            dummies = disjunctive(X=X_quali,prefix=True,sep=".")
            # center and standard deviation
            center2 = Series((dummies.T * row_w).sum(axis=1).to_numpy(),index=dummies.columns,name="center")
            scale2 = Series(sqrt(center2).to_numpy(),index=dummies.columns,name="scale")
            #concatenate
            Xcod = concat_empty(Xcod,dummies,axis=1)
            center = concat_empty(center,center2,axis=0)
            scale = concat_empty(scale,scale2,axis=0)
        ...
        for i in range(n_cols-1):
            for j in range(i+1,n_cols):
                ...
                if is_num1 and is_num2:
                    ...
                    A = Z.columns[Z.columns.str.startswith(k)]
                    B = Z.columns[Z.columns.str.startswith(l)]
                elif is_num1 and not is_num2:
                    A, B = k, Z.columns[Z.columns.str.startswith(l)]
                elif not is_num1 and is_num2:
                    A, B = Z.columns[Z.columns.str.startswith(k)], l
                # similarity matrix
        ...
        for ncl in range(n_cols-2,-1,-1):
            ...
            for i in range(n_cols):
                if (i != cl1) and (i != cl2) and (init_crit[i] == 1):
                    ...
                    for j in clus:
                        k = X.columns[j]
                        is_num = is_numeric_dtype(X[k])
                        if is_num:
                            A.append(k)
                        else:
                            A = [*A,*list(Z.columns[Z.columns.str.startswith(k)])]
                    # final label for idx
                    B = []
                    for j in idx:
                        k = X.columns[j]
                        ...
                        if is_num:
                            ...
                            B = [*B,*list(Z.columns[Z.columns.str.startswith(k)])]
        ...
        for k in uq_cluster:
            ...
            A = []
            for j in clus:
                is_num = is_numeric_dtype(X[j])
                if is_num:
                    A.append(j)
                else:
                    A = [*A,*list(Z.columns[Z.columns.str.startswith(j)])]
        ...
        #---------------------------------------------------------------------------------------------------------------------------------------------------------------------
        # cluster correlation and correlation test
        cluster_cor, cluster_cortest = None, None
        if n_quanti > 0:
            cluster_cor = DataFrame(index=X_quanti.columns,columns=uq_cluster).astype("float")
            for k in cluster_cor.index:
                for i, l in enumerate(uq_cluster):
                    cluster_cor.loc[k,l] = wpearsonr(x=X_quanti[k].values,y=pcs.iloc[:,i],w=row_w).statistic
        ...
        # squared loadings between variables and clusters
        ...
        # convert to dictionary
        var_ = {"cluster" : cluster, "cor" : cluster_cor,"cortest" : cluster_cortest, "sqload" : cluster_sqload, 
