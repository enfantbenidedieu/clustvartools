...
def fviz_dend(obj,
    ...
    **kwargs : Any
    ...
    >>> from clustvartools import CLV, fviz_dend
    >>> clf = CLV(ncl=3)
    >>> clf.fit(decathlon.actif)
    CLV(ncl=3)
    >>> # visualize dendrogram - fill rectangle
    >>> p = fviz_dend(obj=clf,color_labels_by_cluster=True,rect = True,rect_fill = True)
    >>> print(p.show())
    ...
    >>> p = fviz_dend(obj=clf,horiz=True,color_labels_by_cluster=True,rect = True,rect_fill = True)
    >>> print(p.show())
    ...
    # get cluster series
    #---------------------------------------------------------------------------------------------------------------------------------------------------------------------
    if obj.__class__.__name__ in ("CatCLV","CatHCAV"):
        cluster = obj.quali_var_.cluster
    elif obj.__class__.__name__ in ("CLV","HCAV","CorCLV"):
        cluster = obj.quanti_var_.cluster
    elif obj.__class__.__name__ in ("CLVmix","HCAVmix"):
        cluster = obj.var_.cluster
    elif obj.__class__.__name__ == "CatVARHCA":
    ...
    if y_label is None:
        y_label = "Height"
    if title is None:
        title = f"Cluster Dendrogram - {obj.__class__.__name__}"
    if subtitle is None:
        subtitle = ""
