...
def fviz_dend2(Z,
    ...
    text_height : bool, default = False
        If True, annotate the height.
        
    nudge_x : float, default = -0.05
        Horizontal adjustment to apply to the text.
        
    nudge_y : float, default = -0.1
    ...
    **kwargs : Any
    ...
    See also
    --------
    fviz_height : Draw Height
    fviz_dend : Visualization of Dendrogram.

    References
    ----------
    [1] pygal, `Basic Dendrogram <https://anyplot.ai/dendrogram-basic/python/pygal>`_.
    
    [2] pygal. `dendrogram basic <https://anyplot.ai/dendrogram-basic>`_.

    Examples
    --------
    >>> from clustvartools.datasets import decathlon
    >>> from clustvartools import CLV, fviz_dend2
    >>> clf = CLV(ncl=3)
    >>> clf.fit(decathlon.actif)
    CLV(ncl=3)
    >>> p = fviz_dend2(obj=clf.call_.tree.Z,labels = clf.call_.tree.D.index,ncl=3,color_labels_by_cluster=True,rect = True,rect_fill = True)
    >>> print(p.show())
    ...
    if all(x is None for x in (ncl,h)) or (ncl == 1):
        ...
        cluster = Series(fcluster(Z,t=h,criterion=criterion), index=labels,name="cluster",dtype="category")
    elif ncl is not None:
        cluster = Series(fcluster(Z,t=ncl,criterion="maxclust"), index=labels,name="cluster",dtype="category")

    #---------------------------------------------------------------------------------------------------------------------------------------------------------------------
    # set palette
    #---------------------------------------------------------------------------------------------------------------------------------------------------------------------
    uk = sorted(cluster.unique())
    ncl = len(uk)
    if palette is None:
        palette = "Set2"
    ...
    # set color mapping
    colors_mapping = {i : j for i,j in zip(uk,colors)}
    ...
    leaf_df["labels"] = leaf_df["obs"].map({i : j  for i,j in enumerate(labels)})
    ...
    if horiz is False:
        ...
        # show texts
        mapping_aes = {"x":"x", "y":"y", "label":"label"}
        ...
        if text_height:
            height_df = DataFrame({
                "x_coord" : [0.5*sum(icoord[1:3]) for icoord in d["icoord"]],
                "y_coord" : [dcoord[1] for dcoord in d["dcoord"]]
            })
            #
            p = (
                p
                ...
                + geom_text(
                    data = height_df,
                    mapping = aes(x="x_coord",y="y_coord",label="y_coord"),
                    size = text_size,
                    ha = "center",
                    va = "top",
                    nudge_y = nudge_y,
        ...
        if color_segments_by_cluster:
            ...
            p = (
                ...
                + geom_segment(
                    data = segments_df,
                    mapping = aes(y="x", yend="xend", x="y", xend="yend"),
                    color = "black",
                    size = line_size
                )
            )
        ...
        # show texts
        mapping_aes = {"y":"x","x":"y","label":"label"}
        if color_labels_by_cluster and ncl > 1:
            mapping_aes["color"] = "cluster"
        # set texts arguments
        text_args = {
            ...
            if rect:
                if rect_fill:
                    p = (
                        ...
                        + geom_rect(
                            data = rects_df, 
                            mapping = aes(ymin="xmin",ymax="xmax",xmin="ymin",xmax="ymax",fill="cluster"),
                            alpha = rect_alpha
                        )
                        + scale_fill_manual(values=colors_mapping) 
                        + guides(fill=False)
                    ...
                    p = (
                        ...
                        + geom_rect(
                            data = rects_df, 
                            mapping = aes(ymin="xmin",ymax="xmax",xmin="ymin",xmax="ymax"),
                            alpha = rect_alpha, 
                            color = col_border, 
                            fill = "white",
                            linetype = rect_type,
        ...
        if text_height:
            ...
            p = (
                ...
                + geom_point(
                    data = height_df,
                    mapping = aes(y="x_coord",x="y_coord"),
                    inherit_aes=False
                )
                + geom_text(
                    data = height_df,
                    mapping = aes(y="x_coord",x="y_coord",label="y_coord"),
                    size = text_size,
                    ha = "right",
                    va = "center",
                    nudge_x = nudge_x,
